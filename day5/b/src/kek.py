...
for vent_line in vent_lines:
    ...
    elif is_diagonal(vent_line[0], vent_line[1]):

        c1 = vent_line[0] if vent_line[0][0] < vent_line[1][0] else vent_line[1]
        c2 = vent_line[0] if vent_line[0][0] > vent_line[1][0] else vent_line[1]

        gradient = 1 if c1[1] < c2[1] else -1
        dx = c2[0] - c1[0]

        for x in range(dx + 1):
            line_points.append((c1[0] + x, c1[1] + (x * gradient)))
